...
struct clientstate
{
    ...
    {
        if((w_reload(weap, sweap) || weapstore[weap] > 0) && (!check || (weap == weapselect && hasweap(weap, sweap) && weapclip[weap] < W(weap, ammoclip) && weapstate[weap] != W_S_ZOOM && weapwaited(weap, millis, skip))))
            return true;
        return false;
    }
