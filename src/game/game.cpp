    ...
    void clientdisconnected(int cn, int reason)
    {
        ...
        if(d->name[0] && client::showpresence >= (client::waiting(false) ? 2 : 1))
        {
            ...
            if(d->actortype == A_PLAYER)
            {
                ...
                conoutft(CON_EVENT, "\fo%s%s left the game%s (%d %s)", colourname(d), (d->privilege&PRIV_TYPE) < G(iphostlock) ? ipaddr : "", reason >= 0 ? "" : formattedreason, amt, amt != 1 ? "players" : "player");
            }
