...
void renderaction(int idx, int size, Texture *t, char code, const char *name, bool hit)
{
    ...
    if(code) y += draw_textx("\f{%s}", x, y, 0, 0, r, g, b, int((idx ? 255 : f)*compassblend), compassdir[idx].align, -1, -1, getkeyname(code));
    popfont();
    pushfont(!idx || hit ? "emphasis" : "reduced");
    draw_textx("%s", x, y, 0, 0, r, g, b, int(f*compassblend), compassdir[idx].align, -1, -1, name);
    popfont();
}
...
void rendercmenu()
{
    ...
    if(curcompass->actions.length() > 8)
    {
        ...
        loopi(curcompass->actions.length()-8)
        {
            ...
            y += draw_textx("\f{%s} %s", x, y, 0, 0, 255, 255, 255, int(192*compassblend), TEXT_CENTERED, -1, -1, getkeyname(c.code), c.name);
            if(y >= maxy) break;
        }
        popfont();
    }
