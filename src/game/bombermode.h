struct bomberservmode : bomberstate, servmode
{
    ...
    void scorebomb(clientinfo *ci, int relay, int goal)
    {
        ...
        if(m_bb_assault(gamemode, mutators) && G(bomberassaultwinner))
        {
            ...
            if(curbalance == numt-1)
            {
                ...
                if(teamid >= 0)
                {
                    eventlog evt(-1, EV_AFFINITY, EV_A_LIMIT, "S_V_NOTIFY", EV_S_BROADCAST);
                    evt.addclient(ci);
                    evt.addinfo("team", teamid);
                    evt.addinfo("score", teamsc);
    ...
    {
        if(!canplay() || !hasflaginfo || !flags.inrange(i) || ci->state != CS_ALIVE || !(A(ci->actortype, abilities)&AA(AFFINITY))) return;
        flag &f = flags[i];
