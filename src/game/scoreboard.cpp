        ...
        if(groupplayers())
        {
            ...
            if(m_laptime(game::gamemode, game::mutators))
            {
                if(m_team(game::gamemode, game::mutators))
                {
                    ...
                    {
                        pushfont("little");
                        sy += draw_textf("by %s", x+FONTW*2, y, 0, 0, -1, -1, -1, int(blend*255), TEXT_LEFT_UP, -1, -1, 1, game::colourteam(sg.team));
                        popfont();
                        sy += draw_textf("\fg%s", x, y-sy, 0, 0, -1, -1, -1, int(blend*255), TEXT_LEFT_UP, -1, -1, 1, timestr(sg.total, inventoryracestyle));
                    }
                }
                else if(!sg.players.empty())
                {
                    ...
                    {
                        pushfont("little");
                        sy += draw_textf("by %s", x+FONTW*2, y, 0, 0, -1, -1, -1, int(blend*255), TEXT_LEFT_UP, -1, -1, 1, game::colourname(sg.players[0]));
                        popfont();
                        sy += draw_textf("\fg%s", x, y-sy, 0, 0, -1, -1, -1, int(blend*255), TEXT_LEFT_UP, -1, -1, 1, timestr(sg.players[0]->cptime, inventoryracestyle));
                    }
                }
            }
            else if(m_team(game::gamemode, game::mutators))
            {
                if(sg.total)
                {
                    ...
                    sy += draw_textf("by %s", x+FONTW*2, y, 0, 0, -1, -1, -1, int(blend*255), TEXT_LEFT_UP, -1, -1, 1, game::colourteam(sg.team));
                    popfont();
                    sy += draw_textf("\fs\fg%d\fS %s", x, y-sy, 0, 0, -1, -1, -1, int(blend*255), TEXT_LEFT_UP, -1, -1, 1, sg.total, sg.total != 1 ? "laps" : "lap");
                }
            }
            else if(!sg.players.empty() && sg.players[0]->points)
            {
                ...
                sy += draw_textf("by %s", x+FONTW*2, y, 0, 0, -1, -1, -1, int(blend*255), TEXT_LEFT_UP, -1, -1, 1, game::colourname(sg.players[0]));
                popfont();
                sy += draw_textf("\fs\fg%d\fS %s", x, y-sy, 0, 0, -1, -1, -1, int(blend*255), TEXT_LEFT_UP, -1, -1, 1, sg.players[0]->points, sg.players[0]->points != 1 ? "laps" : "lap");
            }
        }
        return sy;
    }
