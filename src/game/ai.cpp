...
namespace ai
{
    ...
    void jumpto(gameent *d, aistate &b, const vec &pos)
    {
        ...
        bool sequenced = d->ai->blockseq || d->ai->targseq, offground = airtime && !physics::liquidcheck(d) && !d->onladder,
             impulse = physics::canimpulse(d, A_A_BOOST, false) && airtime > (b.acttype >= AI_A_LOCKON ? 100 : 250) && !d->turnside && (b.acttype >= AI_A_LOCKON || off.z >= JUMPMIN),
    ...
    void avoid()
    {
        ...
        {
            gameentity &e = *(gameentity *)entities::ents[i];
            if(e.spawned() || e.flags&EF_NOCOLLIDE || !checkmapvariant(e.attrs[13]) || !checkmapeffects(e.attrs[14])) continue;
