...
namespace capture
{
    ...
    void dropaffinity(gameent *d, int i, const vec &droploc, const vec &inertia, int offset)
    {
        ...
        game::event *evt = new game::event(EV_AFFINITY, EV_A_DROP, S_V_FLAGDROP, EV_S_BROADCAST);
        evt->addclient(d);
        evt->addinfo("affinity", i);
        evt->addinfof("console", "\fa%s dropped the the %s flag", game::colourname(d), game::colourteam(f.team, "flagtex"));
        evt->push();
    ...
    void returnaffinity(gameent *d, int i)
    {
        ...
        game::event *evt = new game::event(EV_AFFINITY, EV_A_RETURN, S_V_FLAGRETURN, EV_S_BROADCAST);
        evt->addclient(d);
        evt->addinfo("affinity", i);
        evt->addinfo("millis", millis);
        evt->addinfo("taketime", f.taketime);
        evt->addinfo("droptime", f.droptime);
        evt->addinfo("dropoffset", f.dropoffset);
        evt->addinfof("console", "\fa%s returned the %s flag (time taken: \fs\fc%s\fS)", game::colourname(d), game::colourteam(f.team, "flagtex"), timestr(millis, 1));
        evt->push();
    ...
    void resetaffinity(int i, int value, const vec &pos)
    {
        ...
        if(value > 0)
        {
            ...
            game::event *evt = new game::event(EV_AFFINITY, EV_A_RESET, S_V_FLAGRESET, EV_S_BROADCAST);
            evt->addinfo("affinity", i);
            evt->addinfo("value", value);
            evt->addinfo("taketime", f.taketime);
            evt->addinfo("droptime", f.droptime);
            evt->addinfo("dropoffset", f.dropoffset);
            evt->addinfof("console", "\faThe %s flag has been reset", game::colourteam(f.team, "flagtex"));
            evt->push();
        }
        if(value == 2)
        {
    ...
    void scoreaffinity(gameent *d, int relay, int goal, int score)
    {
        ...
        game::event *evt = new game::event(EV_AFFINITY, EV_A_SCORE, S_V_FLAGSCORE, EV_S_BROADCAST);
        evt->addclient(d);
        ...
        evt->addinfo("millis", millis);
        evt->addinfo("taketime", f.taketime);
        evt->addinfof("console", "\fa%s captured the flag for team %s (score: \fs\fc%d\fS, time taken: \fs\fc%s\fS)", game::colourname(d), game::colourteam(d->team), score, timestr(millis, 1));
        evt->push();
    ...
    void takeaffinity(gameent *d, int i)
    {
        ...
        game::event *evt = new game::event(EV_AFFINITY, EV_A_SECURE, f.team == d->team ? S_V_FLAGSECURED : S_V_FLAGPICKUP, EV_S_BROADCAST);
        evt->addclient(d);
        evt->addinfo("affinity", i);
        evt->addinfof("console", "\fa%s %s the %s flag", game::colourname(d), f.team == d->team ? "secured" : (f.droptime ? "picked up" : "stole"), game::colourteam(f.team, "flagtex"));
        evt->push();
