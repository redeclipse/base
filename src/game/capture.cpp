    ...
    void drawevents(int w, int h, int &tx, int &ty, int tr, int tg, int tb, float blend)
    {
        if(game::focus->state == CS_ALIVE && hud::showevents >= 2)
        {
            ...
            if(!hasflags.empty())
            {
                ...
                ty += draw_textx("You are holding the %s %s", tx, ty, int(FONTW*hud::eventpadx)+FONTW, int(FONTH*hud::eventpady), tr, tg, tb, int(255*blend), TEXT_SKIN|TEXT_CENTERED, -1, -1, str, hasflags.length() == 1 ? "flag" : "flags")*hud::eventscale;
                popfont();
            }
        }
    }
