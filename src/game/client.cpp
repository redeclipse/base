                ...
                case N_CLIENTINIT: // cn colour model checkpoint team priv name vanity count <loadweaps> count <randweaps> handle hostname hostip <version>
                {
                    ...
                    else // new client
                    {
                        ...
                        if(showpresence >= (waiting(false) ? 2 : 1))
                        {
                            ...
                            defformatstring(ipaddr, " (%s)", d->hostname);
                            if(priv > PRIV_NONE)
                            {
                                if(d->handle[0]) conoutft(CON_EVENT, "\fg%s%s joined the game (\fs\fy%s\fS: \fs\fc%s\fS) [%d.%d.%d-%s%d-%s] (%d %s)", game::colourname(d), (d->privilege&PRIV_TYPE) < G(iphostlock) ? ipaddr : "", server::privname(d->privilege), d->handle, d->version.major, d->version.minor, d->version.patch, plat_name(d->version.platform), d->version.arch, d->version.branch, amt, amt != 1 ? "players" : "player");
                                else conoutft(CON_EVENT, "\fg%s%s joined the game (\fs\fy%s\fS) [%d.%d.%d-%s%d-%s] (%d %s)", game::colourname(d), (d->privilege&PRIV_TYPE) < G(iphostlock) ? ipaddr : "", server::privname(d->privilege), d->version.major, d->version.minor, d->version.patch, plat_name(d->version.platform), d->version.arch, d->version.branch, amt, amt != 1 ? "players" : "player");
                            }
                            else conoutft(CON_EVENT, "\fg%s%s joined the game [%d.%d.%d-%s%d-%s] (%d %s)", game::colourname(d), (d->privilege&PRIV_TYPE) < G(iphostlock) ? ipaddr : "", d->version.major, d->version.minor, d->version.patch, plat_name(d->version.platform), d->version.arch, d->version.branch, amt, amt != 1 ? "players" : "player");
                        }
                        if(needclipboard >= 0) needclipboard++;
                        game::specreset(d);
                    }
