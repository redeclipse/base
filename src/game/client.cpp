            ...
            switch(type)
            {
                ...
                case N_SPHY: // simple phys events
                {
                    ...
                    switch(st)
                    {
                        ...
                        case SPHY_BOOST: case SPHY_SLIDE: case SPHY_MELEE: case SPHY_KICK: case SPHY_VAULT: case SPHY_GRAB: case SPHY_PARKOUR:
                        {
                            if(!proceed) break;
                            t->doimpulse(0, IM_T_BOOST+(st-SPHY_BOOST), lastmillis);
                            game::impulseeffect(t);
                            if(st == SPHY_KICK || st == SPHY_VAULT || st == SPHY_GRAB || st == SPHY_PARKOUR || st == SPHY_MELEE)
                                game::footstep(d);
                            break;
                        }
                        case SPHY_COOK:
