...
public:
    ...
    void ingame_process(int controlleridx)
    {
        ...
        if(this->just_pressed(controlleridx)) processkey(this->keymap_id, true);
        else if(this->just_released(controlleridx)) processkey(this->keymap_id, false);
    }
    ...
    void menu_process(int controlleridx)
    {
        ...
        if(this->just_pressed(controlleridx)) UI::keypress(this->keymap_id, true);
        else if(this->just_released(controlleridx)) UI::keypress(this->keymap_id, false);
    }
};
...
void update_from_controller()
{
    ...
    // > available. This is performed automatically by SteamAPI_RunCallbacks,
    // > but for the absolute lowest possible latency, you can call this
...
void update_menu_actions(int controlleridx)
{
    ...
    if(cursor_data.x != 0 || cursor_data.y != 0) lastinputwassiapi = true;
