...
void setupshaders()
{
    ...
    hudbackgroundshader = newshader(0, "<init>hudbackground",
        ...
        "uniform sampler2D tex3; // glimmer\n"
        "varying vec2 pos;\n"
        "fragdata(0) vec4 fragcolor;\n"
        "void main() {\n"
        "    vec2 dir = pos - center;\n"
        ...
        "    float ray = clamp(waterw * -log(-log(texture2D(tex0, uv / 3.0).r)) + caustw * texture2D(tex1, uv).r, 0.0, 1.0);\n"
        "    vec3 fg = foreground * ray;\n"
        "    vec3 bg = cloudw * texture2D(tex2, (pos + 1.0) / 2.0).rgb * foreground;\n"
        "    vec2 guv1 = vec2(pos.x, pos.y - time / 30.0);\n"
        "    vec2 guv2 = vec2(pos.x + 0.3, pos.y - time / 15.0);\n"
        "    float gpf = (1.0 - mod((1.0 + pos.y) / 2.0, 1.0)) * k;\n"
        "    vec4 glim = (texture2D(tex3, guv1) * vec4(foreground, gpf) * 0.25) + (texture2D(tex3, guv2) * vec4(foreground, gpf));\n"
