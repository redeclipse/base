...
void setupshaders()
{
    ...
    hudbackgroundshader = newshader(0, "<init>hudbackground",
        ...
        "    vec3 bg = cloudw * texture2D(tex2, (pos + 1.0) / 2.0).rgb * colour;\n"
        "    vec2 guv1 = vec2(pos.x, pos.y - time / 30.0);\n"
        "    vec2 guv2 = vec2(pos.x + 0.3, pos.y - time / 15.0);\n"
        "    float gpf = 1.0 - mod((1.0 + pos.y) / 2.0, 1.0);\n"
        "    vec4 glim = (texture2D(tex3, guv1) * vec4(colour, gpf) * 0.3) + (texture2D(tex3, guv2) * vec4(colour, gpf * k));\n"
