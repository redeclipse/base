...
static inline bool filterevent(const SDL_Event &event)
{
    switch(event.type)
    {
        case SDL_MOUSEMOTION:
            if(grabinput && !(SDL_GetWindowFlags(screen) & SDL_WINDOW_FULLSCREEN))
            {
                if(warping)
                {
                    ...
                    if (relativemouse)
                    {
                        if (event.motion.x - event.motion.xrel == screenw / 2 && event.motion.y - event.motion.yrel == screenh / 2)
                          return false;
                    }
                    else if (event.motion.x == screenw / 2 && event.motion.y == screenh / 2)
                        return false;
                }
                #ifdef __APPLE__
                if(event.motion.y == 0)
